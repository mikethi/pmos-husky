...
# ════════════════════════════════════════════════════════════════════════════
# Recursive archive / compressed-file extraction
#
# Before images are located we walk the dump directory (and every extracted
# sub-folder) and unpack any archives or compressed files we find.  After
# unpacking into a "<stem>_extracted" sibling folder we immediately recurse
# into that folder so nested archives are also expanded.  This mirrors the
# "check the e folder it extracted too" requirement: every extraction target
# is re-scanned for further extractable content until nothing new is found.
# ════════════════════════════════════════════════════════════════════════════
...
_ARCHIVE_SUFFIXES = (
    ...
    ".lz4",
    ".zst",
    ".simg",            # Android sparse image → expand with simg2img
    ".img.gz",
    ".img.lz4",
    ".img.xz",
    ".img.zst",
    ".bin",             # OTA payload.bin and raw binary blobs
)
...
def _extract_archive(src: Path, dest_dir: Path) -> bool:
    """
    Extract *src* into *dest_dir*.  Returns True on success.
    Uses only stdlib where possible; falls back to subprocess for
    simg2img (sparse images) and lz4/zstd when the Python libs are absent.
    """
    dest_dir.mkdir(parents=True, exist_ok=True)
    name = src.name.lower()
    stem = src.stem  # e.g. "system.img" from "system.img.gz"
    ...
    try:
        ...
        if any(name.endswith(sfx) for sfx in (".tar.gz", ".tgz",
                                               ".tar.bz2", ".tbz2",
                                               ".tar.xz",  ".txz",
                                               ".tar.lz4", ".tar.zst",
                                               ".tar")):
            if name.endswith((".tar.lz4", ".tar.zst")):
                # tarfile cannot handle lz4/zstd; use subprocess
                compress_flag = "--lz4" if name.endswith(".tar.lz4") else "--zstd"
                result = subprocess.run(
                    ["tar", compress_flag, "-xf", str(src), "-C", str(dest_dir)],
                    capture_output=True,
                )
        ...
        if name.endswith(".img.gz") or (name.endswith(".gz") and not name.endswith(".tar.gz")):
        ...
        if name.endswith(".img.bz2") or (name.endswith(".bz2") and not name.endswith(".tar.bz2")):
            out_path = dest_dir / src.stem
            with open(src, "rb") as f_in:
                out_path.write_bytes(bz2.decompress(f_in.read()))
            return True

        if name.endswith(".img.xz") or (name.endswith(".xz") and not name.endswith(".tar.xz")):
            out_path = dest_dir / src.stem
            with lzma.open(src, "rb") as lz_in:
                out_path.write_bytes(lz_in.read())
            return True

        if name.endswith(".img.lz4") or (name.endswith(".lz4") and not name.endswith(".tar.lz4")):
            out_path = dest_dir / src.stem
            if _HAS_LZ4:
                import lz4.frame as _lz4_frame_local
                out_path.write_bytes(_lz4_frame_local.decompress(src.read_bytes()))
                return True
            # Fall back to lz4 CLI
            result = subprocess.run(
                ["lz4", "-d", str(src), str(out_path)],
                capture_output=True,
            )
            return result.returncode == 0

        if name.endswith(".img.zst") or (name.endswith(".zst") and not name.endswith(".tar.zst")):
            out_path = dest_dir / src.stem
            if _HAS_ZSTD:
                dctx = _zstd.ZstdDecompressor()
                out_path.write_bytes(dctx.decompress(src.read_bytes(),
                                                     max_output_size=512 * 1024 * 1024))
                return True
            result = subprocess.run(
                ["zstd", "-d", str(src), "-o", str(out_path)],
                capture_output=True,
            )
            return result.returncode == 0

        if name.endswith(".simg"):
            out_path = dest_dir / (src.stem + ".img")
            result = subprocess.run(
                ["simg2img", str(src), str(out_path)],
                capture_output=True,
            )
            return result.returncode == 0

        if name.endswith(".bin"):
            # payload.bin — OTA payload: extract all partition images from it.
            if name == "payload.bin" or "payload" in name:
                if subprocess.run(["which", "payload-dumper-go"],
                                  capture_output=True).returncode == 0:
                    result = subprocess.run(
                        ["payload-dumper-go", "-output", str(dest_dir), str(src)],
                        capture_output=True,
                    )
                    return result.returncode == 0
                # Fall back to Python payload_dumper
                payload_dumper = subprocess.run(
                    ["which", "payload_dumper"], capture_output=True
                )
                if payload_dumper.returncode == 0:
                    result = subprocess.run(
                        ["python3", payload_dumper.stdout.strip().decode(),
                         "--out", str(dest_dir), str(src)],
                        capture_output=True,
                    )
                    return result.returncode == 0
                print(f"    warn: payload.bin found but no payload dumper available "
                      f"(install payload-dumper-go or payload_dumper)", file=sys.stderr)
                return False
            # Generic .bin: detect format by magic bytes and handle accordingly.
            try:
                raw = src.read_bytes()
            except OSError:
                return False
            # Sparse image inside a .bin
            if len(raw) >= 4 and struct.unpack_from("<I", raw, 0)[0] == SPARSE_MAGIC:
                out_path = dest_dir / (src.stem + ".img")
                result = subprocess.run(
                    ["simg2img", str(src), str(out_path)],
                    capture_output=True,
            ...
            if raw[:2] == b"\x1f\x8b":
                out_path = dest_dir / src.stem
                out_path.write_bytes(gzip.decompress(raw))
                return True
            # XZ-compressed .bin
            if raw[:6] == b"\xfd7zXZ\x00":
                out_path = dest_dir / src.stem
                out_path.write_bytes(lzma.decompress(raw))
                return True
            # bzip2-compressed .bin
            if raw[:2] == b"BZ":
                out_path = dest_dir / src.stem
                out_path.write_bytes(bz2.decompress(raw))
                return True
            # lz4-compressed .bin
            if raw[:4] in (b"\x04\x22\x4d\x18", b"\x02\x21\x4c\x18"):
                if _HAS_LZ4:
                    import lz4.frame as _lz4_frame_local
                    out_path = dest_dir / src.stem
                    out_path.write_bytes(_lz4_frame_local.decompress(raw))
                    return True
                result = subprocess.run(
                    ["lz4", "-d", str(src), str(dest_dir / src.stem)],
                    capture_output=True,
                )
                return result.returncode == 0
            # zstd-compressed .bin
            if raw[:4] == b"\x28\xb5\x2f\xfd":
                ...
                if _HAS_ZSTD:
                    dctx = _zstd.ZstdDecompressor()
                    out_path.write_bytes(dctx.decompress(raw, max_output_size=512 * 1024 * 1024))
                    return True
                result = subprocess.run(
                    ["zstd", "-d", str(src), "-o", str(out_path)],
                    capture_output=True,
                ...
                return result.returncode == 0
            # ZIP magic inside a .bin
            if raw[:4] == b"PK\x03\x04":
                import io as _io
                with zipfile.ZipFile(_io.BytesIO(raw)) as zf:
                    zf.extractall(dest_dir)
                return True
            # Unrecognised .bin — copy as-is so it shows up for manual inspection
            (dest_dir / src.name).write_bytes(raw)
            return True

    except Exception as exc:
        print(f"    warn: extraction failed for {src.name}: {exc}", file=sys.stderr)

    return False


def recursive_extract_archives(root: Path, _visited: set | None = None, depth: int = 0) -> None:
    """
    Walk *root*, extract every archive / compressed file found into a
    sibling ``<stem>_extracted/`` folder, then recurse into that folder
    (and into any other sub-directories) to expand nested archives.

    *_visited* prevents processing the same directory twice.
    """
    if _visited is None:
        _visited = set()
    ...
    for entry in sorted(root.iterdir()):
        if entry.is_file() and _is_extractable(entry):
            dest = entry.parent / (entry.stem.split(".")[0] + "_extracted")
            # Avoid re-extracting if the destination already exists and is non-empty
            if dest.exists() and any(dest.iterdir()):
                recursive_extract_archives(dest, _visited, depth + 1)
                continue
            print(f"{indent}  ↳ extracting {entry.name} → {dest.name}/")
            ok = _extract_archive(entry, dest)
            if ok:
                # Immediately recurse into the newly extracted folder so any
                # nested archives inside are also expanded ("e folder" check).
                recursive_extract_archives(dest, _visited, depth + 1)
            else:
                print(f"{indent}    (extraction skipped — tool not available)", file=sys.stderr)

    # Also descend into existing sub-directories (including previously extracted ones)
    for entry in sorted(root.iterdir()):
        if entry.is_dir():
            recursive_extract_archives(entry, _visited, depth + 1)
...
def process_image(img_path: Path, dump: Path, run_id: int,
    ...
    # Scan the CPIO output directory for any further nested archives or
    # compressed files (the "e folder" check) and expand them in place.
    if files:
        nested_visited: set = set()
        recursive_extract_archives(out_dir, nested_visited, depth=2)
...
def find_images(dump: Path) -> list[Path]:
    """
    Locate all relevant boot / vendor-boot / recovery images under *dump*.

    Before searching, any archives or compressed files in the standard search
    directories are recursively extracted so that images bundled inside a zip
    or factory OTA package are also discovered.
    """
    # ── Step 1: expand any archives in the search directories ────────────────
    search_dirs = [dump / "partitions", dump / "boot_images", dump]
    expand_visited: set = set()
    for search_dir in search_dirs:
        if search_dir.exists():
            print(f"  Scanning for archives in {search_dir.relative_to(dump) if search_dir != dump else '.'}/ ...")
            recursive_extract_archives(search_dir, expand_visited)

    # ── Step 2: collect .img files from all directories (incl. extracted) ───
    found: list[Path] = []
    seen: set = set()
    ...
    def _collect(directory: Path) -> None:
        if not directory.exists():
            return
        # Named priority images first
        for name in IMAGE_NAMES:
            candidate = directory / name
            if candidate.exists() and candidate not in seen:
                found.append(candidate)
                seen.add(candidate)
        # Any other relevant *.img files in this directory
        for img in sorted(directory.glob("*.img")):
            if img not in seen and any(n in img.name for n in ("boot", "recovery", "ramdisk", "init_boot")):
                found.append(img)
                seen.add(img)
        # Recurse into sub-directories (covers *_extracted/ folders)
